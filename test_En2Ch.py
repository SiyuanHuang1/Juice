...
for i in range(0,num):
    ...
    key_list.remove(one_key)
    another_3=random.sample(key_list,3)
    key_list.append(one_key)
    yisi_list=[Words_wordyisi[one_key], Words_wordyisi[another_3[0]], Words_wordyisi[another_3[1]], Words_wordyisi[another_3[2]]]
    random.shuffle(yisi_list) #这一步很重要
    
    #出题
    print("The word is: "+one_key+"\n")
    xuhao_yisi={}
    for j in range(0,4):
        xuhao_yisi[j+1]=yisi_list[j]
        re_pattern=r"\([a-zA-Z]{4,}.*\)"
        yisi_masked=re.sub(re_pattern,"(...)",yisi_list[j])
        print(str(j+1)+"\t"+yisi_masked)
    print()

    #答题
    answer = input("What is your answer?\nInput: ")
    if xuhao_yisi[int(answer)] == Words_wordyisi[one_key]:
...
print("\n\n")
print("Here are some words that you made a wrong choice on: \n")
